...
#         return self.submodel(x, edge_index2norm_adj(edge_index,
#                                                     edge_weights, num_nodes))
...
def edge_index2norm_adj(edge_index, edge_weights=None, num_nodes=None, vector=False):
    ''' convert edge_index and edge_weight into normalised form the original model
     expects '''
    if num_nodes is None:
        num_nodes = edge_index.max().item() + 1
